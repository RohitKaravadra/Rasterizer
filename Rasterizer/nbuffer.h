...
template<std::floating_point T, unsigned int N> // Restricts T to be a floating-point type
class Nbuffer {
...
public:
	...
	Nbuffer(unsigned int w, unsigned int h) {
		create(w, h);
	}
	...
	void create(unsigned int w, unsigned int h) {
		width = w;
		height = h;
		buffer = new T[width * height * N]; // Allocate memory for the buffer
	}
	...
	T& operator () (unsigned int x, unsigned int y) {
		return buffer[((y * width) + x) * N]; // Convert 2D coordinates to 1D index
	}
	...
	void clear() {
		memset(buffer, 0, width * height * N * sizeof(T));
	}
